...
export const apiMiddleware = async (
  ...
  let result = await withErrorHandling(async () => {
    return await withRequestLogging(request, async () => {
      return await withSecurityHeaders(async () => {
        return await withPerformanceMonitoring(async () => {
          return await withDatabaseConnection(async () => {
            let finalHandler = handler;
            
            if (enableRateLimit) {
              finalHandler = () => withRateLimiting(request, finalHandler, rateLimit);
            }
            
            if (enableCache && request.method === 'GET') {
              finalHandler = () => withCache(request, finalHandler, cacheTTL);
            }
...
async function getSystemHealth() {
  ...
  const healthy = memUsage.heapUsed < memUsage.heapTotal * 0.9;
